...
nnoremap  gggHG
vnoremap  "+y
map  :NERDTreeToggle C:\Users\Aaron
noremap  
nnoremap  :update
vnoremap  :update
onoremap  :update
...
noremap  
noremap  u
snoremap <silent>  `>i=XPTemplateStart(0,{'k':'<C-\++'})
...
set runtimepath=~/vimfiles,~\\Vim/vimfiles,~\\Vim\\vimfiles\\bundle\\nerdtree,~\\Vim\\vimfiles\\bundle\\vim-fugitive,~\\Vim\\vimfiles\\bundle\\vim-kolor,~\\Vim\\vimfiles\\bundle\\xptemplate,~\\Vim\\vim74,~\\Vim/vimfiles/after,~/vimfiles/after,~/Vim/vimfiles/bundle/xptemplate/personal
...
silent only
cd ~\CIS1\cursesProject
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +64 main.c
badd +31 \Users\Aaron\CIS1\cursesProject\notes.txt
badd +8 \Users\Aaron\CIS1\cursesProject\Makefile
badd +5 mainMenu.h
badd +9 update.h
badd +7 update.c
badd +1 mainMenu.c
...
setlocal statusline=%-3.3n\ %f\ %h%m%r%w[%{strlen(&ft)?&ft:'none'},%{strlen(&fenc)?&fenc:&enc},%{&fileformat}]%=%{synIDattr(synID(line('.'),col('.'),1),'name')}\ %b,0x%-8B\ %-14.(%l,%c%V%)\ %<%P%{XPMautoUpdate(\"statusline\")}
...
setlocal tabstop=8
setlocal tags=~/CIS1/cursesProject/.git/text.tags,~/CIS1/cursesProject/.git/tags,./tags,tags
...
setlocal wrapmargin=0
silent! normal! zE
let s:l = 31 - ((15 * winheight(0) + 16) / 33)
if s:l < 1 | let s:l = 1 | endif
exe s:l
normal! zt
31
